...
================================================
		...
		str = "\033[3;44;30mTextiحng\033[0m"; // black ,  blue
...
*/
#pragma once 
// توجيه يمنع تكرار تضمين هذا الملف أكثر من مرة عند بناء المشروع

#include <iostream>  // مكتبة الإدخال والإخراج
#include <string>    // للتعامل مع الكائنات النصية string
#include <conio.h>   // للدوال مثل getch() (تستخدم لإدخال حرف بدون Enter)
#include <Windows.h> // للتعامل مع واجهة Windows مثل التحكم في الألوان والموضع
#include "stringo.h" // ملف مخصص - على الأرجح يحتوي على دوال لتنسيق النص

using namespace std; // حتى لا نضطر لكتابة std:: قبل كل شيء

// تعريف كلاس مسؤول عن التنسيق والإخراج والتحكم في شكل الشاشة
class cFormat
{
private:
	// دالة ترسم إطار باستخدام رموز ASCII في الموضع المحدد
	static void genframe(short _x, short _y, short hLen, short vLen, bool defaultShape = true, char hShape = 205, char vShape = 186)
	{
		short horizontalEndFrame = hLen;      // نهاية الإطار أفقيًا
		short verticalEndFrame = vLen;        // نهاية الإطار رأسيًا

		// رسم الخطوط الأفقية العلوية والسفلية
		for (short x = _x; x < horizontalEndFrame; x++)
		{
			cursPosition(x, _y);              // حدد الموضع العلوي
			printf("%c", hShape);             // اطبع رمز الخط الأفقي

			cursPosition(x, verticalEndFrame); // حدد الموضع السفلي
			printf("%c", hShape);
		}

		// رسم الخطوط الرأسية اليمنى واليسرى
		for (short y = _y + 1; y < verticalEndFrame; y++)
		{
			cursPosition(_x, y);               // يسار الإطار
			printf("%c", vShape);

			cursPosition(horizontalEndFrame, y); // يمين الإطار
			printf("%c", vShape);
		}

		if (defaultShape)
		{
			// الزوايا باستخدام رموز ASCII الجاهزة
			cursPosition(_x, _y);                    printf("%c", 201); // الزاوية العلوية اليسرى
			cursPosition(horizontalEndFrame, _y);    printf("%c", 187); // العلوية اليمنى
			cursPosition(_x, verticalEndFrame);      printf("%c", 200); // السفلية اليسرى
			cursPosition(horizontalEndFrame, verticalEndFrame); printf("%c", 188); // السفلية اليمنى
		}
		else
		{
			// الزوايا باستخدام نفس رموز الخطوط
			cursPosition(_x, _y);                    printf("%c", hShape);
			cursPosition(horizontalEndFrame, _y);    printf("%c", hShape);
			cursPosition(_x, verticalEndFrame);      printf("%c", hShape);
			cursPosition(horizontalEndFrame, verticalEndFrame); printf("%c", hShape);
		}
	}
...
public:
	...
	// تعريف ألوان النص والخلفية باستخدام رموز ANSI
	enum enColor {
		txBlack = 30, bgBlack = 40,
		txRed = 31, bgRed = 41,
		txGreen = 32, bgGreen = 42,
	...
	// تغيير لون النص والخلفية
	static string colorbg(string str, enColor txtColor, enColor bgColor = enColor::bgWhite)
	{
		return ("\033[3;" + to_string(bgColor) + ";" + to_string(txtColor) + "m" + str + "\033[0m");
	}

	// إعادة لون الكونسول للوضع الطبيعي
	static void reastColor()
	{
		HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
		SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	...
	// طباعة علم فلسطين باستخدام رموز ملونة
	static void printPalestineFlag(short flagLen = 24, short flagWidth = 48,string shape ="#")
	{
		string flag = "";
		flagLen *= 2;  // مضاعفة الطول لخلق تأثير متدرج
		int g = (flagLen / 2); // نقطة البداية للجزء الثاني
		int w = 0;

		for (short i = 1; i <= flagLen; i+=2)
		{
			if (i <= (flagLen/2)) // الجزء العلوي من العلم
			{
				for (short j = 0; j < i; j++)
				{
					flag += color(shape);
	...
	// تشغيل ملف صوتي (غير مفعّل حاليًا)
	static void playsound(string dpath = "../data/videoplayback.wav")
	{
		// PlaySoundA(dpath.c_str(), NULL, SND_FILENAME | SND_SYNC);
	}

	// إخفاء مؤشر المؤشر في الكونسول
	static void rhideCursor()
	{
		HANDLE console = GetStdHandle(STD_OUTPUT_HANDLE);
		CONSOLE_CURSOR_INFO info;
	...
	// تحديد موضع المؤشر في نافذة الكونسول
	static void cursPosition(short x, short y)
	{
		HANDLE console;
		COORD pos;
	...
	// إنشاء إطار بسيط باستخدام الأحرف
	static void frame(short xPos, short yPos, short hLen, short vLen, bool defaultShape = true, char hShape = 205, char vShape = 186)
	{
		short widthFrame = xPos + hLen;
		short lenFrame = yPos + vLen;
		...
		// إزاحة المؤشر خارج الإطار
		cursPosition(widthFrame+1, lenFrame);
		cout << endl << endl;
	}

	// إنشاء إطار بعنوان يظهر في الأعلى
	static void frame(string title, short xPos, short yPos, short hLen, short vLen)
	{
		short widthFrame = xPos + hLen;
		short lenFrame = yPos + vLen;
		...
		if (widthFrame < title.length())
			widthFrame += (title.length() - widthFrame); // تعديل الحجم ليتسع للعنوان
	...
	// * ملاحظة: دالة inField لم تكتمل في الكود المرسل
};
